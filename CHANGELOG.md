...
All notable changes to `fiskhr` are documented here.
...
- CLI: `fiskhr ovlastenja CERT.p12 [oib]` — asks the F2 service which
  OIBs the certificate holder may report for (OIB defaults to the one in
  the certificate subject); `fiskhr validate --json` prints a
...
- `fiskhr.f2.izvjestavanje` — payment and rejection reporting:
  ...
  `fiskhr.testing.MockEIzvjestavanje` covers all the operations,
...
- `fiskhr.f2.fiskalizacija` — the F2 reporting leg (`EvidentirajERacun`):
  ...
  `fiskhr.f2.ubl.ERacun` model that produced the UBL document, so the
  ...
  **XAdES-B** (ETSI EN 319 132-1) enveloped signatures per spec ch. 11 —
  new `fiskhr.core.xades` module, with the wire format pinned by tests
  ...
  `fiskhr.f2.service`; error table `S001`–`S012` verbatim from the
  schema. `fiskhr.testing.MockEFiskalizacija` answers with XSD-validated,
  signature-verified, signed responses for offline end-to-end testing.

- `fiskhr.f2.ubl` — eRačun construction: `ERacunBuilder` (fluent API),
...
- `fiskhr.f2.validation` — eRačun validation returning a structured
  ...
  Schematron needs the optional `fiskhr[validation]` extra (`saxonche`);
...
- CLI: `fiskhr validate racun.xml` — prints findings with rule ids and
...
- `fiskhr.f1.radno_vrijeme` — working-hours registration (schema v1.10):
...
- CLI: `fiskhr zki` (offline ZKI computation, `--legacy-sha1` for the
  transition period) and `fiskhr echo` (CIS connectivity test,
  `--env demo|production`).
...
- `fiskhr.f1.client.FiskalizacijaClient` — the public F1 surface:
...
- `fiskhr.core.transport.SoapClient` — SOAP 1.1 over httpx with a TLS 1.2
...
- `fiskhr.testing.MockCis` — public in-process CIS mock that validates
...
- `fiskhr.f1.models` — Pydantic v2 models mirroring FiskalizacijaSchema
...
- `fiskhr.f1.messages` — `build_racun_zahtjev` / `parse_racun_odgovor`;
  every built document is validated against the vendored official XSD in
  tests, signed and unsigned.
- `fiskhr.core.xmldsig` — enveloped XML-DSig signing and verification per
...
- `fiskhr.core.signing.SignatureMethod` — RSA-SHA256 / RSA-SHA1 with the
  migration timeline documented, shared by ZKI and (upcoming) XML-DSig.
- `fiskhr.f1.service` — authoritative demo/production endpoint URLs
  (spec §6.1) and `SCHEMA_VERSION`; documents that the official PROD WSDL
  bundle ships with the test URL in `soap:address`.
- `fiskhr.f1.error_codes.CIS_ERROR_MESSAGES` — the full s001–s013 error
...
- Project scaffold: src layout, `pyproject.toml` (hatchling + uv), ruff,
  ...
  Python 3.11–3.13.
- `fiskhr.core.types`: OIB validation (ISO 7064 MOD 11,10), verified
  against publicly known real OIBs.
- `fiskhr.core.errors`: structured exception hierarchy (`code`,
  `message_hr` on every error).
- `fiskhr.core.certs.Certificate`: P12/PFX loading, validity checks,
  best-effort OIB extraction from the subject; private key excluded from
  `repr()`, passwords never retained or echoed.
- `fiskhr.f1.zki`: offline ZKI computation (`izracunaj_zki`,
  `zki_payload`, `format_iznos`).
- CLI: `fiskhr --version`, `fiskhr cert info <file.p12>` (password via
  `FISKHR_P12_PASSWORD` or interactive prompt — never a CLI argument).
