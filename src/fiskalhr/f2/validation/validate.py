...
from fiskalhr.f2.validation.report import ValidationReport
from fiskalhr.f2.validation.schematron import validate_schematron
from fiskalhr.f2.validation.xsd import parse_document, validate_xsd
...
def validate(xml: bytes | etree._Element, *, schematron: bool = True) -> ValidationReport:
    ...
    Args:
        ...
        schematron: Run the business rules (requires the
            ``fiskalhr[validation]`` extra). Set False for XSD-only checks.
