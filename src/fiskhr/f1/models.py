...
XML element names lives in `fiskhr.f1.messages`.
...
`fiskhr.f1.zki.format_iznos`. Never use floats for money.
"""
...
from fiskhr.core.types import validate_oib
...
class Racun(_Model):
    ...
    Field order mirrors the schema's ``<sequence>``; `fiskhr.f1.messages`
