"""The ``fiskhr`` command-line interface.
...
P12 passwords are read from the ``FISKHR_P12_PASSWORD`` environment variable
...
PASSWORD_ENV_VAR = "FISKHR_P12_PASSWORD"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    echo_parser.add_argument("text", nargs="?", default="fiskhr echo", help="text to echo")
    ...
    validate_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Schematron rules (requires the fiskhr[validation] extra)."
        ),
    )
...
def _echo(args: argparse.Namespace) -> int:
    ...
    from fiskhr.f1.client import _SOAP_ACTION_BASE
    from fiskhr.f1.messages import build_echo_request, parse_echo_response
    from fiskhr.f1.service import SERVICE_URLS
...
def _ovlastenja(args: argparse.Namespace) -> int:
    ...
    if oib is None:
        print(
            ...
            "pass it explicitly: fiskhr ovlastenja CERT.p12 <oib>",
            file=sys.stderr,
        )
        return 2
