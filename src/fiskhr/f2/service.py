...
def efiskalizacija_schema_path() -> Path:
    ...
    return Path(str(files("fiskhr.f2") / "schemas" / "efiskalizacija" / "eFiskalizacijaSchema.xsd"))


def eizvjestavanje_schema_path() -> Path:
    """Path to the vendored eIzvještavanje XSD."""
    return Path(str(files("fiskhr.f2") / "schemas" / "eizvjestavanje" / "eIzvjestavanjeSchema.xsd"))
