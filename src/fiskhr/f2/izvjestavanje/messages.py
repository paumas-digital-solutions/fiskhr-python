...
XAdES-B signature via `fiskhr.core.xades.sign_xades_enveloped`.

The response ``Odgovor`` structure is identical to eFiskalizacija's, so
parsing reuses `fiskhr.f2.fiskalizacija`'s response models — and the
...
from fiskhr.f2.fiskalizacija.messages import _eracun_element, format_datum_vrijeme
